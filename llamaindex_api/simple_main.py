"""
Simple FastAPI Chatbot with LlamaIndex Workflow Checkpointing (No OpenAI required)
"""
...
from simple_chatbot_workflow import SimpleChatbotManager

# Initialize FastAPI app
app = FastAPI(
    title="Simple LlamaIndex Chatbot API",
    description="Stateless chatbot with conversation checkpointing (No OpenAI required)",
    version="1.0.0"
)

# Initialize simple chatbot manager
...
async def root():
    """Root endpoint"""
    return {
        "message": "Simple LlamaIndex Chatbot API",
        "version": "1.0.0",
        "description": "No OpenAI API key required for testing",
        "endpoints": {
            "chat": "POST /chat",
...
async def chat(request: ChatRequest):
    ...
    """
    try:
        print(f"🔍 DEBUG: Received chat request: session_id={request.session_id}, message='{request.message}'")
...
if __name__ == "__main__":
    print("🚀 Starting Simple LlamaIndex Chatbot API...")
